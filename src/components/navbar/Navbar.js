...
function Navbar() {
  ...
  const [data, setData] = useState([{"id":1,"title":"Fjallraven - Foldsack No. 1 Backpack, Fits 15 Laptops","price":109.95,"description":"Your perfect pack for everyday use and walks in the forest. Stash your laptop (up to 15 inches) in the padded sleeve, your everyday","category":"men's clothing","image":"https://fakestoreapi.com/img/81fPKd-2AYL._AC_SL1500_.jpg","rating":{"rate":3.9,"count":120}},
  ...
  function updateSearchResults(userInput){

    // const userInput = event.target.value;
    // console.log("userInput", userInput)
    if(userInput.length >0 && userInput.length%2 ==0)
    {
      data.map( obj => {

        const stringValue = (obj.title+" "+obj.category).toLowerCase();
        
        stringValue.includes(userInput)? setSearchList(prev => [ ...prev.filter( item => item.id !==obj.id),obj]) : false
      })
  ...
  return (
    <>
      {showLoginModal &&
        createPortal(<Login closeModal={closeModal} />, document.body)}
        {/* {createPortal(<div id='searchResults'></div>, document.body)} */}
      <div className="navbar__wrapper">
        ...
        <div className="login__cart">
          ...
          { searchList.length>0 && createPortal(<SearchResults searchList={searchList} clearSearchList={clearSearchList}/> , document.body)}
    </>
  );
}
